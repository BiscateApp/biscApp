...
export class PortfolioComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPortfolio): number => this.portfolioService.getPortfolioIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.portfolios = this.refineData(dataFromBody);
  }

  protected refineData(data: IPortfolio[]): IPortfolio[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IPortfolio[] | null): IPortfolio[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.portfolioService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
