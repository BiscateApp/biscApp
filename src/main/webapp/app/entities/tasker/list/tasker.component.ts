...
import { EntityArrayResponseType, TaskerService } from '../service/tasker.service';
...
export class TaskerComponent implements OnInit {
  ...
  trackId = (_index: number, item: ITasker): number => this.taskerService.getTaskerIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.taskers = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ITasker[] | null): ITasker[] {
    // If there is previus link, data is a infinite scroll pagination content.
    if ('prev' in this.links) {
      const taskersNew = this.taskers ?? [];
      if (data) {
        for (const d of data) {
          if (taskersNew.map(op => op.id).indexOf(d.id) === -1) {
            taskersNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.taskerService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
