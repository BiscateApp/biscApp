...
type RestOf<T extends IJob | NewJob> = Omit<T, 'date' | 'whenCreated' | 'whenUpdated'> & {
  date?: string | null;
...
export class JobService {
  ...
  protected convertDateFromClient<T extends IJob | NewJob | PartialUpdateJob>(job: T): RestOf<T> {
    return {
      ...
      whenCreated: job.whenCreated?.toJSON() ?? null,
      whenUpdated: job.whenUpdated?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restJob: RestJob): IJob {
    return {
      ...
      whenCreated: restJob.whenCreated ? dayjs(restJob.whenCreated) : undefined,
      whenUpdated: restJob.whenUpdated ? dayjs(restJob.whenUpdated) : undefined,
    };
  }
