...
/**
 * REST controller for managing {@link com.bisc.app.domain.Job}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Job> result = jobRepository
            ...
            .map(existingJob -> {
                ...
                if (job.getWhenCreated() != null) {
                    existingJob.setWhenCreated(job.getWhenCreated());
                }
                if (job.getWhenUpdated() != null) {
                    existingJob.setWhenUpdated(job.getWhenUpdated());
                }
