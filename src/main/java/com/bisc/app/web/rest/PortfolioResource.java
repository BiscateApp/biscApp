...
/**
 * REST controller for managing {@link com.bisc.app.domain.Portfolio}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PortfolioResource {
        ...
        Optional<Portfolio> result = portfolioRepository
            ...
            .map(existingPortfolio -> {
                if (portfolio.getBio() != null) {
                    existingPortfolio.setBio(portfolio.getBio());
                }
                if (portfolio.getSkills() != null) {
                    existingPortfolio.setSkills(portfolio.getSkills());
                }
                if (portfolio.getSpeakingLanguages() != null) {
                    existingPortfolio.setSpeakingLanguages(portfolio.getSpeakingLanguages());
                }
                if (portfolio.getStars() != null) {
                    existingPortfolio.setStars(portfolio.getStars());
                }
                if (portfolio.getCompletedTasks() != null) {
                    existingPortfolio.setCompletedTasks(portfolio.getCompletedTasks());
                }
                if (portfolio.getHourlyRate() != null) {
                    existingPortfolio.setHourlyRate(portfolio.getHourlyRate());
                }
    ...
    public List<Portfolio> getAllPortfolios(@RequestParam(required = false) String filter) {
        if ("tasker-is-null".equals(filter)) {
            log.debug("REST request to get all Portfolios where tasker is null");
            return StreamSupport
                .stream(portfolioRepository.findAll().spliterator(), false)
                .filter(portfolio -> portfolio.getTasker() == null)
                .toList();
        }
